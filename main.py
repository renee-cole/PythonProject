...
FLU_assembly_time = 4
Package_time = 1
DISPATCH_CONTROL_TIME_2 = 8
DISPATCH_CONTROL_TIME_1 = 4
STOCK_CONTROL_TIME_2 = 8
STOCK_CONTROL_TIME_1 = 16
...
class vaccineFacility(object):
    def __init__(self, env):
        ...
        self.ingredient1_stock_control = env.process(
            self.stock_control(env, self.Ingredient1, 'Ingredient 1', ingredient1.cost))
        self.ingredient2_stock_control = env.process(
            self.stock_control(env, self.Ingredient2, 'Ingredient 2', ingredient2.cost))
        self.ingredient3_stock_control = env.process(
            self.stock_control(env, self.Ingredient3, 'Ingredient 3', ingredient3.cost))
        self.ingredient4_stock_control = env.process(
            self.stock_control(env, self.Ingredient4, 'Ingredient 4', ingredient4.cost))
        self.ingredient5_stock_control = env.process(
            self.stock_control(env, self.Ingredient5, 'Ingredient 5', ingredient5.cost))
        self.ingredient6_stock_control = env.process(
            self.stock_control(env, self.Ingredient6, 'Ingredient 6', ingredient6.cost))
        self.ingredient7_stock_control = env.process(
            self.stock_control(env, self.Ingredient7, 'Ingredient 7', ingredient7.cost))
        self.ingredient8_stock_control = env.process(
            self.stock_control(env, self.Ingredient8, 'Ingredient 8', ingredient8.cost))
        self.ingredient9_stock_control = env.process(
            self.stock_control(env, self.Ingredient9, 'Ingredient 9', ingredient9.cost))
        self.ingredient10_stock_control = env.process(
            self.stock_control(env, self.Ingredient10, 'Ingredient 10', ingredient10.cost))
    ...
    def COVID_process1(self):
        ...
        yield self.env.timeout(random.gauss(self.COVID_process1_time, 1))
    ...
    def COVID_process2(self):
        ...
        yield self.env.timeout(random.gauss(self.COVID_process2_time, 1))
    ...
    def COVID_process3(self):
        ...
        yield self.env.timeout(random.gauss(self.COVID_process3_time, 1))
    ...
    def FLU_process3(self):
        ...
        yield self.env.timeout(random.gauss(self.FLU_process3_time, 1))
    ...
    def FLU_process_assembly(self):
        ...
        yield self.env.timeout(random.gauss(self.FLU_assembly_time, 1))
    ...
    def packaging(self, C=True):
        ...
        yield self.env.timeout(random.gauss(self.package_time, 1))
        if C is True:
    ...
    def stock_control(self, env, ingred, name, cost):
        ...
        while True:
            if ingred.level <= C1_critical_stock:
                ...
                print('----------------------------------')
                yield env.timeout(random.gauss(STOCK_CONTROL_TIME_1, 1))
                ...
                print('----------------------------------')
                yield env.timeout(random.gauss(STOCK_CONTROL_TIME_2, 1))
            else:
                yield env.timeout(1)
    ...
    def dispatch_control(self, env, vaccine, C=True):
        ...
        while True:
            ...
            if vaccine.level >= 50:
                ...
                print('----------------------------------')
                yield env.timeout(DISPATCH_CONTROL_TIME_1)
                ...
                print('----------------------------------')
                yield env.timeout(DISPATCH_CONTROL_TIME_2)
            else:
                yield env.timeout(1)
    ...
    def demand_control(self, env):
        while True:
            if self.unvaccinated_pop > 0:
                ...
                elif (self.vaccinated_pop / self.total_population <= (
                        self.percent_health_workers + self.percent_at_risk)):
                    self.COVID_order_amount = 3 * self.percent_at_risk * self.total_population / (8 * 7 * 8)

                # stage 3: vaccinate adults in 3 months
                elif (self.vaccinated_pop / self.total_population <= (
                        self.percent_health_workers + self.percent_at_risk + self.percent_adults)):
...
def setup(env):
    ...
    while vf.unvaccinated_pop > 0:
        ...
        yield env.timeout(random.gauss(COVID_order_interval, 1))
        i += vf.COVID_order_amount
        env.process(COVID_ingredients(env, 'Covid #%d' % i, vf))
        env.process(COVID_assembler(env, 'Covid #%d' % i, vf))
        env.process(COVID_packager(env, 'Covid #%d' % i, vf))

        yield env.timeout(random.randint(FLU_order_interval, 1))
        i += FLU_order_amount
        env.process(FLU_ingredients(env, 'Flu #%d' % i, vf))
        env.process(FLU_assembler(env, 'Flu #%d' % i, vf))
        env.process(FLU_packager(env, 'Flu #%d' % i, vf))
...
print('It took an average of %.2f for each COVID vaccine to be produced.' % (np.mean(COVID_prep_time_list)
                                                                             + np.mean(
            COVID_assembly_time_list) + np.mean(COVID_package_time_list)))
print('It took an average of %.2f for each FLU vaccine to be produced.' % (np.mean(FLU_prep_time_list)
                                                                           + np.mean(FLU_assembly_time_list) + np.mean(
            FLU_package_time_list)))
